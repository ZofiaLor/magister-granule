...
data = pandas.read_csv("wyniki/corners_accuracy.csv")
...
for i in range(3):
    for j in range(3):
        ...
        plt.plot(d['ksi'], d['accuracy'], color=colors[i*3+j])
...
print(to_group.groupby(by=["data size", "granules number", "relation type"]).mean())
