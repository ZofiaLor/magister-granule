...
class FuzzyCMeans(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.cluster_centers_ = np.random.uniform(low=np.min(X), high=np.max(X), size=(self.n_clusters, X.shape[1]))

        # Initialize fuzzy membership matrix randomly
        # self.U_ = np.random.dirichlet(np.ones(self.n_clusters), size=n_samples)
        self.U_ = np.random.uniform(size=(n_samples, self.n_clusters))
        self.U_ = normalize(self.U_, axis=1, norm='l1')
