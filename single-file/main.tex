...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{SingleLinkage}{$\zX, \nKlaster$}
		\State $\nX \gets $ liczba danych
		\State $d \gets $ \Call{distance}{$\zX$} \Comment{macierz $\nX \times \nX$ odległości między każdą parą danych}
		\State $l \gets [1, 2, ..., \nX-1, \nX]$ \Comment{etykiety}
		\For{$i = 1$ to $\nX - \nKlaster$} 
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State $\nAtrybut \gets $ liczba atrybutów \Comment{wymiary $\zX$} 
		\State $\wCentre \gets $ \Call{InitCenters}{$\zX$} \Comment{Inicjalizacja wektora środków klastrów \ksremark{Trzeba zdefiniować funkcje InitCenters}}
		...
		\State aktualizacja wartości $\wCentre$ na podstawie wzoru \ref{eq:center_update}
...
\begin{enumerate}
	\item Granulacja oryginalnych danych przy pomocy algorytmu rozmytych $c$-średnich.
...
Grupowanie hierarchiczne ma postać zmodyfikowanego algorytmu naiwnego \texttt{PRIMITIVE\_CLUSTERING()} \ksremark{$\gets$ dałbym do makra}, o złożoności czasowej $O(n^{3})$. Zmieniony musi zostać sposób wyznaczania kolejnych minimalnych odległości w macierzy $\mDistance$ oraz, zależne od łączności, aktualizacje odległości do nowo scalonego klastra. 

Minimum i maksimum wyznacza się przy pomocy operacji porównania. W przypadku rozszerzalnych liczb rozmytych, wynikami takich operacji są liczby z przedziału $[0, 1]$. W celu określenia, która z liczb rozmytych jest większa lub mniejsza, wprowadzony zostaje parametr $\xi$, będący paramtetrem badawczym. Algorytmy rozmytego minimum i maksimum przedstawiają odpowiednio pseudokody \ref{alg:fuzzy-min} i \ref{alg:fuzzy-max}.
...
\subsection{Badanie jakości grupowania}

Jakość zmierzono poprzez porówanie wyników algorytmu AgglomerativeClustering biblioteki scikit-learn z własną implementacją.
...
Rozmyty algorytm grupowania jako wynik zwraca przynależność do klastrów granul, nie oryginalnych danych. Określono więc, że na potrzeby porównania, punkt należy do granuli, dla której jego wartość w tablicy $\mClusterMembership$ jest maksymalna.

Wykorzystana miara dokładności przyjmuje, że algorytm ostry, dla łączności \SingleLinkage{}, ma dokładność 100\% -- każdy punkt zostaje przypisany do oczekiwanego klastra.
...
\begin{align}
	\recall_{\klaster} = \frac{l_{\klaster\klaster}}{\sum_{i=0}^{\nKlaster}l_{\klaster i}},
\end{align}

\begin{align}
	\precision_{\klaster} = \frac{l_{\klaster\klaster}}{\sum_{i=0}^{\nKlaster}l_{i \klaster}}.
\end{align}
...
W przypadku \SingleLinkage{}, najlepszą dokładność, trafność i precyzję zaobserwowano dla danych \crescents, a najgorszą -- dla danych \laguna. Wyniki przeanalizowano również na podstawie wykresów.
...
\begin{tabular}{rrr}
\toprule
	kształt & dokładność \SingleLinkage{} & dokładność \CompleteLinkage{} \\
\midrule
	    \blobs & 97,931\% & 86,663\% \\
		\circles & 87,911\% & 56,908\% \\
		\corners & 80,708\% & 51,514\% \\
		\crescents & 99,915\% & 95,702\% \\
		\laguna & 79,749\% & 44,209\% \\
		\spheres & 97,210\% & 70,242\% \\
\bottomrule
\end{tabular}
\end{table}  
...
\midrule
	    \blobs & 73,257\% & 75,492\% \\
		\circles & 82,430\% & 49,790\% \\
		\corners & 80,708\% & 51,514\% \\
		\crescents & 99,915\% & 95,702\% \\
		\laguna & 61,674\% & 45,171\% \\
		\spheres & 89,581\% & 40,323\% \\
\bottomrule
\end{tabular}
\end{table}  
...
\midrule
	    \blobs & 72,061\% & 68,967\% \\
		\circles & 79,112\% & 49,623\% \\
		\corners & 84,673\% & 48,214\% \\
		\crescents & 99,948\% & 96,579\% \\
		\laguna & 58,617\% & 37,230\% \\
		\spheres & 88,814\% & 44,232\% \\
\bottomrule
\end{tabular}
\end{table}  
...
W przypadku danych \circles (rys. \ref{fig:circles1000e}) największe znaczenie dla dokładności ma liczba granul. Dla mniejszej liczby (50) rozmyte odległości między granulami położonymi na zewnętrznym okręgu są większe niż między okręgiem zewnętrznym a wewnętrznym. Okrąg zewnętrzny jest też bardziej liczny, niż wewnętrzny (proporcja 7:3). Dysproporcja ta nie jest tak duża, jak w przypadku \blobs, więc różnica między wartością dokładności a wartościami trafności i precyzji jest mniejsza.
...
Dane \crescents tworzą dwa klastry o identycznej liczebności i gęstości, dzięki czemu dokładność, trafność i precyzja ich grupowania są najlepsze (rys. \ref{fig:crescents1000e}). Ze względu na bliskie położenie końców łuków, przy dużym rozmyciu wciąż możliwe są błędy, jak w przypadku 50 granul, $\xi=0.9$.

Dla danych \laguna (rys. \ref{fig:laguna1000e}), podobnie jak w przypadku \circles, błędy wynikają z większego rozmycia granul położonych na otaczającym pozostałe dwa klastry okręgu. Okrąg ma większą od nich liczebność, proporcja danych wynosi 8:1:1. Przekłada się to na znacznie niższe wartości trafności i precyzji w porównaniu z dokładnością.

W przypadku danych \spheres (rys. \ref{fig:spheres1000e}) można zauważyć, że wyniki poprawiają się wraz ze zwiększeniem liczby granul i zmniejszeniem wartości parametru $\xi$. Proporcja liczebności klastrów wynosi 9:1, więc trafność i precyzja różnią się znacznie od dokładności.
...
Podobnie jak w przypadku \SingleLinkage{}, najlepsze wyniki dla łączności \CompleteLinkage{} uzyskano dla danych \crescents, a najgorsze -- dla danych \laguna. Wszystkie wyniki są znacznie gorsze od uzyskanych dla \SingleLinkage{}.
...
\midrule
	trójkątna & 92,101\% & 66,955\% \\
	wykładnicza & 90,549\% & 67,488\% \\
	gaussowska & 89,063\% & 68,176\% \\
\bottomrule
\end{tabular}
\end{table} 
...
\midrule
	trójkątna & 83,936\% & 59,607\% \\
	wykładnicza & 81,218\% & 59,710\% \\
	gaussowska & 78,629\% & 59,679\% \\
\bottomrule
\end{tabular}
\end{table} 
...
\midrule
	trójkątna & 82,828\% & 57,464\% \\
	wykładnicza & 80,540\% & 57,436\% \\
	gaussowska & 78,245\% & 57,522\% \\
\bottomrule
\end{tabular}
\end{table} 
...
Dla \SingleLinkage{}, najwyższą dokładność -- 93,7716\% -- osiągnięto dla $\xi=0,1$, wartości metryk spadają wraz ze wzrostem wartości parametru.
