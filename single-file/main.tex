...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State $L \gets [] $ \Comment{Lista wynikowa}
		\State $rozmiar[x] \gets 1$ dla wszystkich $x \in S$
		\For{$i = 0$ to $N-2$} 
		...
		\State $S \gets S \setminus \{a, b\}$
		\State nowy element $n \notin S$
		\For{$x$ in $S$}
		\State $\mDistance[n, x] = \mDistance[x, n] \gets $ \Call{UpdateFormula}{...} \Comment{wzór dla odpowiedniej łączności}
		\EndFor
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State $U \gets $ losowe wartości początkowe przynależności punktów do klastrów, spełniające warunek ze wzoru \ref{eq:cluster_mem_condition}
		\State $i\gets 1$
...
\begin{enumerate}
	...
	\item przekształcenie otrzymanych wyników w granule o potrzebnych parametrach (roz. \ref{sec:granulacja-danych-oryginalnych}),
	\item aglomeracyjne grupowanie hierarchiczne granul z uwzględnieniem ich rozmycia: \MojHierAlg\ (roz. \ref{sec:granulacja-danych-oryginalnych}).
\end{enumerate}
...
Wartości ostre są tożsame ze środkami klastrów będących wynikiem algorytmu rozmytych $c$-średnich, czyli z wektorem $\wCentre$ (\ref{eq:center_update}). \ksremark{tu odnośnik do odpowiedniego wzoru w poprzednim rozdziale}
...
\begin{algorithm}[t]
	...
	\label{alg:fuzzy-min}
	\begin{algorithmic}[1]
		\Require $a, b$: porównywane rozszerzalne liczby rozmyte, $r$: typ relacji (trójkątna, wykładnicza, gaussowska), $\xi$: parametr porównania 
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		\Require $\mX$: macierz liczb rozmytych, $r$: typ relacji (trójkątna, wykładnicza, gaussowska), $\xi$: parametr porównania 
		\Procedure{FuzzyArgMin}{$\mX, r, \xi$}
		\State $result \gets \infty_0$
		\State $(w, k) \gets (0, 0)$
		\For{$i = 1$ to $\nX - 1$}
		\For{$j = i + 1$ to $\nX$}  
		\If{$(\mX[i, j] <_{\nPodobienstwo_r} result) > \xi$}
		\State $result \gets \mX[i, j]$
		\State $(w, k) \gets (i, j)$
		\EndIf
		\EndFor
		\EndFor
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State $\mDistance \gets $ \Call{distance}{$\zX$} \Comment{macierz $\nX \times \nX$ odległości między każdą parą danych}
		\State $\wEtykieta \gets [1, 2, ..., \nX-1, \nX]$ \Comment{etykiety}
		\For{$i = 1$ to $\nX - \nKlaster$} 
		\State $w, k \gets $ \Call{FuzzyArgMin}{$\mDistance$} \Comment{scalenie najbliższych klastrów} 
		\For{$i = 1$ to $\nX$}
		\State $\mDistance[w, i] = \mDistance[i, w] = $ \Call{FuzzyMinimum}{$\mDistance[w, i], \mDistance[k, i]$} \Comment{przypisanie wynikające z łączności}
		\State $\mDistance[k, i] = \mDistance[i, k] = \infty_0 $ \Comment{nieużywane w dalszych iteracjach}
		\EndFor
		\State zamień etykiety $k$ na $w$
		\EndFor
		\State $mapa \gets$ lista unikalnych wartości w $\wEtykieta$
		\For{$i = 1$ to $\nX$} 
		\State $\wEtykieta[i] \gets$ indeks $\wEtykieta[i]$ w tablicy $mapa$ \Comment{mapowanie etykiet na kolejne liczby naturalne} \ksremark{$\gets$ nie rozumiem}
		\EndFor
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\For{$i = 1$ to $\nKlaster - 1$}
		\State $(w, k) \gets$ $\arg\max$(l[i:][i:]) \Comment{Indeksy wiersza i kolumny, w których znajduje się maksymalna wartość podmacierzy $\mConfusionMatrix[i:][i:]$}
		\State $w \gets w + i$
		\State $k \gets k + i$
...
Wyznaczono macierz $\mConfusionMatrix$, w której wartość elementu $\ConfusionMatrix_{ij}$ jest równa liczbie punktów należących do klastra $i$ wyznaczonego przez algorytm ostry i do klastra $j$ wyznaczonego przez algorytm rozmyty:
\begin{align}
