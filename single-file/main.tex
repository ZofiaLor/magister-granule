...
Kolejne 2 klastry, które mają zostać scalone w danej iteracji, wybierane są na podstawie metryki odległości między nimi oraz łączności (\english{linkage}).

Odwrotnością algorytmu aglomeracyjnego jest algorytm deglomeracyjny (\english{divisive}), w którym na początku wszystkie dane znajdują się w jednym klastrze, który jest iteracyjnie dzielony na mniejsze, aż do uzyskania klastrów wyłącznie jednoelementowych.
...
\end{align}
gdzie $n=$ liczba atrybutów punktu danych $x$, $x_{i,k}$ oznacza $k$-ty atrybut $i$-tego punktu.
...
W algorytmie \english{single linkage} odległość między klastrami jest równa minimalnej odległości między dwoma punktami należącymi do tych klastrów.
...
\section{Obliczenia granularne}

Liczby rozmyte
Granule
Kształty (trójkątne, wykładnicze, gaussowskie)
Relacje między liczbami (arytmetyka, porównanie)
