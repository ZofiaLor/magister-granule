...
Algorytm zbadano pod kątem szybkości wykonania oraz jakości grupowania poprzez porównanie jego efektów z efektami algorytmów non-fuzzy.
...
Czas pojedynczego wykonania algorytmu non-fuzzy zmierzono poprzez różnicę czasu rozpoczęcia i zakończenia grupowania hierarchicznego. Wykorzystana została własna implementacja algorytmu grupowania, analogiczna do wersji fuzzy.
W czasie pojedynczego wykonania algorytmu fuzzy zawarto czas wykonania algorytmu fuzzy C-means, utworzenia na jego podstawie listy granul oraz rozmytego grupowania hierarchicznego.
...
Pomiary wykonano dla wszystkich kształtów i rozmiarów danych. W przypadku algorytmu fuzzy, pomiary przeprowadzono również dla różnych liczebności granul (50, 100, 200), typu relacji liczb rozmytych (trójkątne, wykładnicze, gaussowskie) oraz parametru $\xi$ (0,05-0,95 z krokiem 0,05).
...
Obliczenie dokładności polegało na wyznaczeniu jej z macierzy pomyłek $l$ przy założeniu, że algorytm non-fuzzy posiada dokładność 100\%.
W celu uzyskania macierzy pomyłek, do każdego elementu $l_{ij}$ przypisano liczbę punktów należących do klastra $i$ według algorytmu non-fuzzy i do klastra $j$ według algorytmu fuzzy, gdzie $i, j \in \langle 0, N-1 \rangle , N = \text{liczba klastrów}$.
Wyznaczono taką permutację kolumn i wierszy macierzy, która maksymalizuje wartości na przekątnej.
...
\caption{Charakterystyka czasu wykonania algorytmów non-fuzzy i fuzzy dla różnej liczby granul od liczby danych wejściowych (circles).}
\label{fig:circles-times}
\end{figure}

Dla obydwu algorytmów, złożoność czasowa grupowania hierarchicznego wynosi $O(n^{3})$, jednak w przypadku algorytmu fuzzy dotyczy ona tylko granul, których liczba jest stała.
