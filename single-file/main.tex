...
Grupowanie (klasteryzacja) danych polega na przypisaniu elementów do klas o pewnym znaczeniu, gdzie elementy tej samej klasy są podobne %ze względu na określoną metrykę
\cite{bib:fcm-for-optimizing,bib:clustering-alg-survey} \ksremark{, a równocześnie niepodobne do obiektów z innych grup}. Algorytmy grupowania są algorytmami nienadzorowanymi -- mają jedynie informacje o podobieństwie między poszczególnymi elementami.
...
\begin{itemize}
	...
	\item Algorytmy oparte o minimalizację funkcji kryterialnej: związane z iteracyjnym wyznaczaniem środków klastrów do momentu spełnienia kryterium zatrzymania (na przykład: k-means \ksremark{$k$-means, fuzzy $c$-means}, fuzzy c-means),
	\item Algorytmy gęstościowe: obszary o dużej gęstości danych tworzą klastry, pozostałe uznawane są za szumy (na przykład: DBSCAN \cite{bib:dbscan-algorithm}), \ksremark{jeszcze jakiś przykład}
	\item Algorytmy grafowe: dane uznawane są za wierzchołki grafów, a podobieństwa między nimi -- za krawędzie, \ksremark{jakiś przykład}
	\item Inne algorytmy. \ksremark{jakiś przykład}
\end{itemize}
...
Proces hierarchicznego scalania (lub rozdzielania) klastrów można zwizualizować jako dendrogram (rys. \ref{fig:dendrogram-iris}). \ksremark{Trzeba napisać trochę szerzej o dendrogramie. Co oznacza wysokość łączenia itd. itp.}


\ksremark{No i tutaj dajemy pseudokod – coś w stylu alg. \ref{alg:cascade_inference}.}
...
\caption{Dendrogram otrzymany na podstawie grupowania hierarchicznego o łączności \english{single linkage} przykładowego zbioru danych ,,\english{iris}''.}
\label{fig:dendrogram-iris}
\end{figure}
\ksremark{W opisie rys. \ref{fig:dendrogram-iris} napiszemy \iris.}
...
W algorytmie \english{single linkage} \ksremark{Proponowałbym użycie makr do formatowania nazw algortymów, np. \SingleLinkage.} \cite{bib:hierarchical-clustering} odległość między klastrami jest równa minimalnej odległości między dwoma punktami należącymi do tych klastrów.
...
\end{align}
%
Przykład takiego przypisania ilustruje rys. \ref{fig:single-linkage-example}.
...
Algorytm \english{complete linkage} jest odwrotnością \english{single linkage}: odległość między klastrami jest równa maksymalnej odległości między dwoma punktami należącymi do tych klastrów. W momencie złączenia dwóch klastrów w jeden, jego odległość od pozostałych klastrów określa się więc jako:
\begin{align}
\Odleglosc \left(A, B\right) = \max(\Odleglosc(A_{1}, B), \Odleglosc(A_{2}, B)).
\end{align}
%
Algorytm dobrze działa dla kulistych, gęstych klastrów, ale w porównaniu z \english{single linkage} zwraca gorsze wyniki dla danych o innych kształtach \cite{bib:hierarchical-clustering, bib:sklearn-linkage-comparison}.
...
\ksremark{Tu i dalej we wzorach trzeba użyć $\max$ zamiast $max$.}
...
Dane znajdują się w plikach \url{.data} zlokalizowanych w folderze \url{dane}, pogrupowanych w podfoldery według kształtów.
...
Dane przedstawiają współrzędne lokalizacji logowania użytkowników platform społecznościowych.
Dane przycięto, aby zawierały jedynie kolumny ,,latitude'' oraz ,,longitude''.  
\begin{figure}
\centering
...
\end{tabular}
\end{table}  
\ksremark{Tab. \ref{id:tab:zbiory}: Gdzieś tam wyżej zdefiniowałem przykładowy zapis dla zbioru \iris. Trzeba podobnie zrobić dla pozostałych zbiorów, żebyśmy mieli wspólny jednolity zapis dla wszystkich zbiorów danych.}

\section{Metodyka badań}

Algorytm \ksremark{Tu proszę wymyślić jaką chwytliwą nazwę :-)} zbadano pod kątem szybkości wykonania oraz jakości grupowania poprzez porównanie jego efektów z efektami algorytmu ostrego \ksremark{konkretnie jakiego?}.
...
Pomiary wykonano dla wszystkich kształtów i rozmiarów danych. W przypadku algorytmu rozmytego, pomiary przeprowadzono również dla różnych łączności (\english{single linkage, complete linkage}), liczebności granul (50, 100, 200), typu relacji liczb rozmytych (trójkątne, wykładnicze, gaussowskie) oraz parametru $\xi$ (0,05-0,95 z krokiem 0,05) \ksremark{Można to zapisać też $\xi \in \{ 0.05, 0.10, 0.15, \ldots, 0.95 \}$}.
...
Wykorzystana miara dokładności przyjmuje, że algorytm ostry, dla łączności \english{single linkage}, posiada \ksremark{lepiej: ma} dokładność 100\% -- każdy punkt zostaje przypisany do oczekiwanego klastra.
...
\end{align}
\ksremark{Lepiej jest nieużywać kilkuliterowych oznaczeń, bo formalnie $acc$ oznacza iloczyn $a \cdot c \cdot c$. Lepiej będzie prowadzić jakiś pojedynczy symbol.}
\ksremark{`*' oznacza splot, dla mnożenia mamy $\cdot$ lub $\times$.}
lub
\begin{align}
acc = \frac{\sum_{i=0}^{X}l_{ii}}{\sum_{i=0}^{X}\sum_{j=0}^{X}l_{ij}} * 100\%,
\end{align}
gdzie $X=$ liczba klastrów. \ksremark{Nie zmieniamy oznaczeń :-) Użyjemy $\nKlaster$.}

\section{Wyniki}

\subsection{Szybkość algorytmu}

Algorytm, który wykorzystuje wstępną granulację fuzzy C-means, posiada \ksremark{lepiej: ma} znacząco mniejszy czas wykonania w stosunku do algorytmu non-fuzzy \ksremark{Trzeba napisać to trochę inaczej. Nie używaliśmy wcześniej frazy `algorytm non-fuzzy'.}.
...
Dokładność grupowania danych w zależności od ich kształtu dla łączności \english{single linkage} przedstawiono w tabeli \ref{id:tab:acc-ksztalt}.
...
\begin{table}
\centering
\caption{Dokładność grupowania hierarchicznego danych single linkage w zależności od ich kształtu}
...
Wiąże się to z większym rozmyciem granul na ,,ramionach'' klastrów. \ksremark{Na pewno rozmycie na ramionach jest większe?}
...
Dokładność grupowania danych w zależności od ich kształtu dla łączności \english{complete linkage} przedstawiono w tabeli \ref{id:tab:acc-cl-ksztalt}.
Podobnie jak w przypadku \english{single linkage}, najlepsze wyniki uzyskano dla danych ,,crescents'', a najgorsze -- dla danych ,,laguna''. Wszystkie wyniki są znacznie gorsze od uzyskanych dla \english{single linkage}.

\begin{table}
\centering
\caption{Dokładność grupowania hierarchicznego danych complete linkage w zależności od ich kształtu}
...
Średnią dokładność grupowania danych w zależności od typu relacji między granulami przedstawiono w tabeli \ref{id:tab:acc-rt} dla łączności \english{single linkage} i w tabeli \ref{id:tab:acc-cl-rt} dla \english{complete linkage}.
Wyniki są zbliżone, jednak najlepszą dokładność odnotowano dla relacji trójkątnej i łączności \english{single linkage}. Wiąże się to ze skończonym nośnikiem liczb rozmytych.

\begin{table}
\centering
\caption{Dokładność grupowania hierarchicznego danych single linkage w zależności od typu relacji}
\label{id:tab:acc-rt}
\begin{tabular}{rr}
\toprule
...
\begin{table}
\centering
\caption{Dokładność grupowania hierarchicznego danych complete linkage w zależności od typu relacji}
...
Średnią dokładność grupowania danych w zależności od parametru $\xi$ przedstawiono na rysunkach \ref{fig:xi-acc-sl} i \ref{fig:xi-acc-cl}.

Dla \english{single linkage}, najwyższą dokładność -- 93,7716\% -- osiągnięto dla $\xi=0,1$, dokładność spada wraz ze wzrostem wartości parametru.
Zależność jest przeciwna dla \english{complete linkage}: najlepsze wyniki -- dokładność 69,546\% -- osiągnięto dla $\xi=0,95$.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{./xi_acc_sl.pdf}
\caption{Zależność dokładności grupowania dla łączności single linkage od wartości parametru $\xi$}
\label{fig:xi-acc-sl}
\end{figure}
...
\caption{Zależność dokładności grupowania dla łączności complete linkage od wartości parametru $\xi$}
\label{fig:xi-acc-cl}
\end{figure}

\ksremark{Rys. \ref{fig:xi-acc}: Może tak będzie lepiej?}
