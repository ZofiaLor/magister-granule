...
Kolejne 2 \ksremark{słownie: dwa} klastry, które mają zostać scalone w danej iteracji, wybierane są na podstawie metryki odległości między nimi oraz łączności (\ksremark{ang.} \english{linkage}).

Odwrotnością algorytmu aglomeracyjnego jest algorytm deglomeracyjny (\english{divisive}), w którym na początku wszystkie dane znajdują się w jednym klastrze, który jest iteracyjnie dzielony na mniejsze, aż do uzyskania klastrów wyłącznie jednoelementowych. \ksremark{$\gets$ Do będzie w rozdziale \ref{sec:grupowanie}}
...
\begin{align}
d(x_{i}, x_{j}) = (\sum_{k=0}^{n}|x_{i,k} - x_{j,k}|^{p})^{1/p},
\end{align}
...
Do łączności zaimplementowanych w popularnych bibliotekach należą \english{single-linkage} (najbliższe sąsiedztwo), \english{complete linkage} (najdalsze sąsiedztwo), \english{average linkage} (średnie sąsiedztwo) oraz metoda Warda (minimalna wariancja).
...
W algorytmie \english{single linkage} \ksremark{cytowanie} odległość między klastrami jest równa minimalnej odległości między dwoma punktami należącymi do tych klastrów.
...
\begin{align}
d(A, B) = min(d(A_{1}, B), d(A_{2}, B)) \ksremark{kropka na końcu, bo wzór kończy zdanie}
\end{align}
...
Możliwe implementacje algorytmu przedstawione są w \cite{bib:hierarchical-algorithms}.

%przepisać, przekopiować pseudokody?
Algorytm ogólny (\texttt{GENERAL\_LINKAGE()}) może być wykorzystany dla każdej łączności. Jego wadą jest pesymistyczna złożoność czasowa wynosząca $O(n^{3})$.
