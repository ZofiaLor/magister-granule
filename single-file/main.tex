...
Problemem algorytmów grupowania hierarchicznego jest ich duża złożoność czasową \ksremark{czasowa} -- w przypadku algorytmu ogólnego aż do $O(n^{3})$. Zmniejszenie liczby danych poprzez zastosowanie wstępnej granulacji algorytmem rozmytych $c$-średnich może znacznie przyspieszyć jego działanie, jako że algorytm ten ma stałą złożoność czasową w zależności od liczby danych ($O(n)$). \ksremark{Napisałbym tutaj, że ma złożoność $O(\nGranula^3)$, ale $\nGranula \ll n$.}

Algorytm taki, nazwany \MojAlg, składa się z trzech głównych kroków:
\ksremark{Rozmyty algorytm aglomeracyjnego grupowania hierarchicznego \MojAlg\ składa się z trzech głównych kroków:}

\begin{enumerate}
	\item granulacja oryginalnych danych przy pomocy algorytmu rozmytych $c$-średnich \ksremark{(roz. \ref{sec:granulacja-danych-oryginalnych})}.
	\item przekształcenie otrzymanych wyników w granule o potrzebnych parametrach \ksremark{jak wyżej}
	\item aglomeracyjne grupowanie hierarchiczne granul z uwzględnieniem ich rozmycia: \MojHierAlg\ \ksremark{jak wyżej}.
\end{enumerate}
...
Wartości ostre są tożsame ze środkami klastrów będących wynikiem algorytmu rozmytych $c$-średnich, czyli z wektorem $\wCentre$. \ksremark{tu odnośnik do odpowiedniego wzoru w poprzednim rozdziale}
...
Zbiór granul \ksremark{$\zGranula$} oraz ich kształt są parametrami wejściowymi algorytmu rozmytego aglomeracyjnego grupowania hierarchicznego \ksremark{nazwa algorytmu}.
%
Na początku wyznaczane są rozmyte odległości euklidesowe (wzór \eqref{eq:odleglosc:euklidesowa})między każdą parą granul, przy pomocy operacji arytmetycznych określonych w sekcji \ref{sec:rozszerzalne-liczby-rozmyte}. Wynikiem obliczeń jest symetryczna macierz odległości $\mDistance$ o wymiarach $\nX \times \nX$, gdzie $\Distance_{ij} \ksremark{= \Distance_{ji}} = \Distance(\wX_{i}, \wX_{j})$.

Grupowanie hierarchiczne \MojHierAlg{} ma postać zmodyfikowanego algorytmu naiwnego \texttt{PRIMITIVE\_CLUSTERING()} \ksremark{$\gets$ dałbym do makra}, o złożoności czasowej $O(n^{3})$ \ksremark{Tu musimy uważać: $O(\nGranula^3)$}. Zmieniony musi zostać sposób wyznaczania kolejnych minimalnych odległości w macierzy $\mDistance$ oraz, zależne od łączności, aktualizacje odległości do nowo scalonego klastra. 

Minimum i maksimum wyznacza się przy pomocy operacji porównania. W przypadku rozszerzalnych liczb rozmytych, wynikami takich operacji są liczby z przedziału $[0, 1]$. W celu określenia, która z liczb rozmytych jest większa lub mniejsza, wprowadzony zostaje parametr $\xi$, będący parametrem badawczym. Algorytmy rozmytego minimum i maksimum przedstawiają odpowiednio pseudokody \ref{alg:fuzzy-min} i \ref{alg:fuzzy-max}.
...
Rozmyty algorytm grupowania jako wynik zwraca przynależność do klastrów granul, nie oryginalnych danych. Określono więc, że na potrzeby porównania, punkt należy do tej granuli, dla której jego wartość w tablicy $\mClusterMembership$ jest maksymalna.
...
\begin{algorithm}[t]
	\caption{Wyznaczenie macierzy $l$}
	...
	\begin{algorithmic}[1]
		...
		\State inkrementacja $l[\wEtykieta_{r}[i]][\wEtykieta_{g}[\ClusterMembership]]$
		\EndFor
		\State \Return $l$
		\EndProcedure
	\end{algorithmic}
...
\begin{algorithm}[t]
	\caption{Permutacja macierzy $l$}
	\label{alg:perm_l_matrix}
	\begin{algorithmic}[1]
		\Require $l$: macierz $\nKlaster \times \nKlaster$, wyznaczona przez algorytm \ref{alg:calc_l_matrix}
		\Procedure{PermuteLMatrix}{$l$}
		\For{$i = 1$ to $\nKlaster - 1$}
		\State $(w, k) \gets$ \Call{argmax}{$l[i:][i:]$} \Comment{Indeksy wiersza i kolumny, w których znajduje się maksymalna wartość podmacierzy $l[i:][i:]$}
		\State $w := w + i$
		\State $k := k + i$
		\State zamiana kolejnością wiersza $i$-tego i $w$-tego macierzy $l$
		\State zamiana kolejnością kolumny $i$-tej i $k$-tej macierzy $l$
		\EndFor
		\State \Return $l$
		\EndProcedure
	\end{algorithmic}
...
Wyznaczono macierz $l$ \ksremark{Musimy trzymać się spójnego zapisu. Dotychczas macierze oznaczaliśmy wielkimi wytłuszczonymi symbolami.}, w której wartość elementu $l_{ij}$ jest równa liczbie punktów należących do klastra $i$ wyznaczonego przez algorytm ostry i do klastra $j$ wyznaczonego przez algorytm rozmyty:
\begin{align}
...
Numeracja klastrów wyznaczonych przez obydwa algorytmy może być inna ze względu na różną kolejność ich wyznaczenia. Wykorzystując algorytm przedstawiony na pseudokodzie \ref{alg:perm_l_matrix}, wyznaczono taką permutację wierszy i kolumn macierzy $l$, która maksymalizuje wartości na przekątnej. 

Z uzyskanej w ten sposób macierzy pomyłek, dokładność wyznaczana jest jako suma wartości na przekątnej macierzy podzielona przez całkowitą liczbę danych: \ksremark{bez mnożenia przez 100\%}
\begin{align}
...
W przypadku danych \circles\ (rys. \ref{fig:circles1000e}) największe znaczenie dla dokładności ma liczba granul. Dla mniejszej liczby (50) rozmyte odległości między granulami położonymi na zewnętrznym okręgu są większe niż między okręgiem zewnętrznym a wewnętrznym. Okrąg zewnętrzny jest też bardziej liczny, \nocomma\ niż wewnętrzny (proporcja 7:3). Dysproporcja ta nie jest tak duża, jak w przypadku \blobs, więc różnica między wartością dokładności a wartościami trafności i precyzji jest mniejsza.

Klastry dla danych \corners\  mają nierównomierną gęstość, przez co algorytm fuzzy C-means \ksremark{Trzeba ujednolicić zapis nazwy algorytmu.} tworzy więcej granul bliżej środka układu współrzędnych (rys. \ref{fig:corners1000e}).
