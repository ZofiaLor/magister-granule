...
Algorytm ten powinien przyspieszyć proces grupowania, jednocześnie zachowując wysoką jego jakość.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\For{$j$ in $1..\nX$}\Comment{scalenie etykiet łączonych wierszy}
		\If{$\wEtykieta[j] == k$}
		\State $\wEtykieta[j] = w$ \Comment{zamiana etykiet}
		\EndIf
		\EndFor
		\EndFor
        \State \Comment{przemianowanie etykiet $\wEtykieta$ na kolejne wartości:  \ksremark{przemyśleć :-)}}
