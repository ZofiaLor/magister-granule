...
\newcommand{\MojAlg}{\procedura{AggloGr}}
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		\Require $X$: dane do klasteryzacji, $c$ docelowa liczba klastrów
		\Procedure{SingleLinkage}{$X, c$}
		\State $N \gets $ liczba danych
		\State $d \gets $ \Call{distance}{X} \Comment{macierz NxN odległości między każdą parą danych}
		\State $l \gets [1, 2, ..., N-1, N]$ \Comment{etykiety}
		\For{$i = 1$ to $N - c$} 
		\State $w, k \gets $ wiersz i kolumna $\min(d)$ \Comment{scalenie najbliższych klastrów} 
		\For{$i = 1$ to $N$}
		...
		\EndFor
		\State mapowanie etykiet na kolejne liczby naturalne
		\State \Return $l$
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[t]
	...
	\label{alg:fcm}
	\begin{algorithmic}[1]
		\Require $X$: dane do klasteryzacji, $n$: liczba klastrów docelowych, $m$: współczynnik rozmycia, $i$: maksymalna liczba iteracji, $\epsilon$: tolerancja
		\Procedure{FuzzyCMeans}{$X, n, m, i, \epsilon$}
		\State $p \gets $ liczba próbek 
		\State $a \gets $ liczba atrybutów \Comment{wymiary $X$}
		\State $v \gets $ \Call{InitCenters}{X} \Comment{Inicjalizacja wektora środków klastrów}
		\State $U \gets $ \Call{InitMembership}{} \Comment{Inicjalizacja macierzy przynależności wartościami losowymi przy zachowaniu warunku poprawności}
		\For{$i = 1$ to $i$} 
		\State aktualizacja wartości $v$ na podstawie wzoru \ref{eq:center_update}
		\State aktualizacja wartości $U$ na podstawie wzoru \ref{eq:membership_update}
...
\subsection{Granulacja danych oryginalnych}
...
\subsection{Przekształcenie granul}
...
Na początku wyznaczane są rozmyte odległości euklidesowe \ref{eq:odleglosc:euklidesowa} między każdą parą granul, przy pomocy operacji arytmetycznych określonych w sekcji \ref{sec:rozszerzalne-liczby-rozmyte}. Wynikiem obliczeń jest symetryczna macierz odległości $\mDistance$ o wymiarach $\nX \times \nX$, gdzie $\Distance_{ij} = \Distance(\wX_{i}, \wX_{j})$.

Grupowanie hierarchiczne ma postać zmodyfikowanego algorytmu naiwnego \texttt{PRIMITIVE\_CLUSTERING()}, o złożoności czasowej $O(n^{3})$. Zmieniony musi zostać sposób wyznaczania kolejnych minimalnych odległości w macierzy $\mDistance$ oraz, zależne od łączności, aktualizacje odległości do nowo-scalonego klastra. 

Minimum i maksimum wyznacza się przy pomocy operacji porównania. W przypadku rozszerzalnych liczb rozmytych, wynikami takich operacji są liczby z przedziału $[0, 1]$. W celu określenia, która z liczb rozmytych jest większa lub mniejsza, wprowadzony zostaje parametr $\xi$, będący paramtetrem badawczym. Algorytmy rozmytego minimum i maksimum przedstawiają pseudokody \ref{alg:fuzzy-min} i \ref{alg:fuzzy-max}.
