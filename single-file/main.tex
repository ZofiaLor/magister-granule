...
\item Wyznaczenie dokładności (accuracy) grupowania danych rozmytych poprzez utworzenie macierzy pomyłek. Wynik: dokładność jest gorsza, niż w przypadku danych oryginalnych. Zależność dokładności od:
	\begin{itemize}
	\item Kształtu zbioru danych (blobs, corners itd.): Algorytm działa najlepiej, gdy docelowe klastry mają zbliżoną liczebność i równomierną gęstość. 
	...
	\item Typu relacji (t, e, g): Wynik niekonkluzywny, ale w wielu przypadkach relacja trójkątna skutkuje wyższą dokładnością.
	\item Parametru ksi (0.05, 0.1, ... , 0.9, 0.95): Wynik niekonkluzywny, lepsze wyniki zazwyczaj uzyskiwane są dla niższych wartości parametru, ale dokładność znacznie fluktuuje.
	\end{itemize}
...
\begin{itemize}
\item Porównanie czasu wykonania grupowania dla danych o mniejszych różnicach (np. co 500: 500, 1000, 1500, ... 5000).
\item Różne liczby iteracji algorytmu fuzzy C-means (teraz wyniki są tylko dla 80 iteracji).
\item Dokładniejsze porównanie wpływu typu relacji na dokładność grupowania.
\item Duże zbiory danych, np. gowalla i brightkite.
\end{itemize}
%
% 
%
