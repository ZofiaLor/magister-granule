...
Graficzną interpretację tych charakterystyk przedstawiono na rys. \ref{fig:core-support-example}. Zbiór rozmyty może nie posiadać rdzenia ($\forall \X \in \zA  \ClusterMembership(\X)_{\zA} < 1$), może mieć nieskończony nośnik ($\forall \X \in \zA  \ClusterMembership(\X)_{\zA} > 0$).
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\For{$i = 1$ to $\nPunkt - 1$}
		\For{$j = i + 1$ to $\nPunkt$}
		\State $pi, pj \gets \wPunkt[i], \wPunkt[j]$  
		\If{($\mX[pi, pj] <_{\nPodobienstwo_r}$ result) > $\xi$}
		\State result $\gets \mX[pi, pj]$
		\State $(w, k) \gets (pi, pj)$
		\EndIf
		\EndFor
		\EndFor
		\State \Return $(w, k)$
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\For{$i = 1$ to $\nX - \nKlaster$} 
		\State $w, k \gets $ \Call{FuzzyArgMin}{$\mDistance, \wPunkt, r, \xi$} \Comment{scalenie najbliższych klastrów} 
		...
		\EndFor
		\State zamień etykiety o wartości $k$ na $w$
		\EndFor
