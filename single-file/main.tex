...
Grupowanie danych, nazywane też klasteryzacją, jest powszechnie wykorzystywane do analizy danych. Istnieje wiele rodzajów algorytmów klasteryzacyjnych, między innymi hierarchiczne, gęstościowe, grafowe. Niezależnie od typu, polegają one na łączeniu podobnych do siebie danych w grupy (klastry).

\ksremark{tutaj o alg. grupowania hierarchicznego}
\ksremark{o ich złożoności}

W pracy rozważany będzie algorytm aglomeracyjnego grupowania hierarchicznego. Polega on na iteracyjnym łączeniu najbliższych sobie mniejszych klastrów w większe, aż do uzyskania z góry określonej ich liczby. Złączanie odbywa się na podstawie wybranych metryk odległości klastrów. Problemem tej rodziny algorytmów jest ich wysoka złożoność czasowa w zależności od ilości danych wejściowych -- $O(n^3)$ w implementacji naiwnej, choć istnieją specjalizowane implementacje o lepszych złożonościach.  

\ksremark{tutaj o obliczeniach granularnych}

Do ulepszenia takiego algorytmu można wykorzystać obliczenia granularne. Jest to szeroka dziedzina pozwalająca na analizę danych na sposób zbliżony do ludzkiego pojmowania rzeczywistości. Pozwalają na to na przykład zbiory rozmyte, czyli takie zbiory, do których dane mogą należeć w pewnym stopniu i każda dana może należeć do wielu zbiorów jednocześnie. 
...
Celem niniejszej pracy będzie opracowanie i przebadanie własności granularnego algorytmu aglomeracyjnego grupowania hierarchicznego, który operować będzie na ziarnach informacyjnych uzyskanych w wyniku klasteryzacji algorytmem rozmytych $c$-średnich.

\ksremark{Co na tym zyskujemy? Obniżenie złożoności}
Algorytm ten ma niższą złożoność czasową -- liniową w zależności od liczby danych -- powinien więc przyspieszyć proces grupowania. Jednocześnie zachowana powinna zostać wysoka jakość grupowania.
...
{bib:fcm-for-optimizing,bib:clustering-alg-survey}, a równocześnie niepodobne do obiektów z innych grup. Algorytmy grupowania są algorytmami nienadzorowanymi -- mają jedynie informacje o podobieństwie między poszczególnymi elementami.
...
Przykładem ilustrującym ideę obliczeń granularnych w jednym wymiarze może być subiektywna ocena temperatur jako ,,zimne'' lub ,,ciepłe''. Temperaturę $10^\circ C$ trudno jednoznacznie zaklasyfikować do jednej z tych dwóch kategorii, dla niektórych osób będzie ona ,,zimna'', dla niektórych ,,ciepła''. Temperatura $20^\circ C$ dużo bardziej pasuje do kategorii ,,ciepła'' niż ,,zimna'', jednak w mniejszym stopniu, niż $25^\circ C$. Choć w uzyskaniu bardziej szczegółowych informacji może pomóc dodanie większej liczby kategorii, na przykład ,,umiarkowana'' i ,,gorąca'', to nie odzwierciedli ono subiektywnej oceny poszczególnych wartości.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\EndFor
		\EndFor
        \State \Comment{przemianowanie etykiet $\wEtykieta$ na kolejne liczby naturalne:  \ksremark{przemyśleć :-)}}
